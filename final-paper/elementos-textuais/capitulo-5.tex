...
\begin{figure}[htb]
	...
	\caption{À esquerda os volumes delimitadors se intersectam porém os objetos não colidem. À direita os volumes delimitadores se intersectam e os objetos colidem.}
	\label{fig:fecho-convexo-interseccao}
\end{figure}

A principal motivação para o uso de volumes delimitadores é que formas mais simples (como caixas ou esferas) permitem testes de sobreposição muito mais baratos do que a geometria original que envolvem. Além disso, se os objetos se intersectam então os seus volumes delimitadors se intersectam, como ilustrado na Figura~\ref{fig:fecho-convexo-interseccao}. Esse fato pode ser usado para acelerar os testes de colisão, como veremos posteriormente. 
...
A caixa delimitadora alinhada aos eixos (AABB) é um dos volumes delimitadores mais comuns. Trata-se de um paralelepípedo (ou retângulo, em 2D) cujas faces são paralelas aos eixos do sistema de coordenadas. Seu teste de interseção é simples como no Algoritmo~\ref{alg:interseccao-aabb}.
\begin{algorithm}[htb]
...
\subsection*{Caixa Delimitadora Orientada (OBB)}
...
A fase de filtragem é chamada de \textit{Broad Phase} e tem como objetivo descartar rapidamente pares de objetos que seguramente não estão colidindo. Segundo \citeonline{ericson2004real}, ``nada é mais rápido do que não ter que realizar uma tarefa''. Portanto, a melhor otimização é reduzir a quantidade de trabalho o mais cedo possível.

Como objetos só podem colidir com outros que estejam próximos, a \textit{Broad Phase} utiliza consultas espaciais para identificar apenas aqueles que compartilham regiões próximas do espaço. O teste mais simples usado nesta etapa é o teste de interseção entre volumes delimitadores primitivos como no Algoritmo~\ref{alg:broad-phase}, devido ao seu baixo custo computacional.
...
Um esquema muito eficaz de subdivisão espacial consiste em dividir um espaço em várias células de tamanho igual formando uma grade. Cada objeto é então associado às células com as quais se sobrepõem, como ilustrado na Figura~\ref{fig:grade-uniforme}.

\textbf{VETORIZAR FIGURA}
\begin{figure}[htb]
...
\textbf{EXPLICAR INICIALIZAÇÃO DA GRADE UNIFORME}
...
\textbf{INSERIR ALGORITMO DE GRADE UNIFORME}

Após filtrar a lista de pares, a \textit{Narrow Phase} realiza testes geométricos precisos. Esta fase utiliza algoritmos complexos como:
\begin{itemize}
