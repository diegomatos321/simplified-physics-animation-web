...
% Assim, o objetivo do GJK reduz para determinar se a CSO contém ou não a origem. 
\begin{figure}[htb]
	\centering
...
Usaremos a mesma notação de \citeonline{linahan2015geometricgjk}, $||P||$ para denotar a norma (distância à origem) de um ponto $P$. O algoritmo constrói iterativamente simplices dentro do CSO mais próximos da origem até que o simplex pare de mudar. O ponto $P$ de norma mínima dessas simplices serve como aproximações sucessivas para \textit{MinimumNormPoint(CSO)}.
...
\begin{algorithm}[htb]
	...
	\While{$||P||^2 - P \cdot V > \epsilon^2$}{
		$Q' \leftarrow Q \cup \{V\}$ \\
		P $\leftarrow$ MinimumNormPoint(ConvexHull($Q'$)) \\
...
A função de suporte deve retornar o ponto mais distante em uma direção dentro da CSO. Isso cria um simplex que contém uma área máxima, aumentando, portanto, a chance de que o algoritmo termine rapidamente. Além disso, podemos usar o fato de que todos os pontos retornados desta forma estão na borda da CSO e, portanto, se não pudermos adicionar um ponto além da origem ao longo de alguma direção, sabemos que a CSO não contém a origem. Isso aumenta as chances de o algoritmo sair rapidamente em casos de não interseção.
