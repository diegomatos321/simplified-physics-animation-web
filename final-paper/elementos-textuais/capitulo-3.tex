...
O SAT é um algoritmo genérico rápido que pode remover a necessidade de ter código de detecção de colisão para cada par tipo de forma, reduzindo assim o código e a manutenção. O SAT busca pelas sombras formadas ao traçar raios paralelos sob os dois objetos, de diversas posições diferentes, se as sombras estiverem separadas o algoritmo responde que as formas não estão sobrepondo. Ele se baseia no teorema geométrico que afirma:
...
\begin{figure}[htb]
	...
	\caption{Eixo separador perpendicular a linha vertical tracejada.}
...
$$
	A - B = \{\vec{x} - \vec{y} \mid \vec{x} \in A, \vec{y} \in B \},
$$
que pode ser pensado como um processo de varredura que calcula o vetor distância para cada ponto de $B$ em $A$. Essa operação continua sendo a soma de Minkowski (a soma da diferença) mas, neste trabalho, usaremos esse termo para referir a essa operação quando necessário. 

Dessa forma, caso a distância entre dois pontos seja zero, podemos confirmar há colisão entre os dois objetos. Esse vetor também coincide com a origem, logo elaboramos nosso próximo teorema:
\begin{teorema}
