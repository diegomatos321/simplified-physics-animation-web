...
A versão de \citeonline{ericson2004gjk} substituiu a versão algébrica de Johnson por uma geométrica, usando rotinas geométricas primitivas como \textit{ClosestPointOnEdgeToPoint()} e \textit{ClosestPointOnTriangleToPoint()}, que é implementado usando operações vetoriais. Apesar disso seu algoritmo é matematicamente equivalente ao de Johnson com um pequeno acrescimo de otimização: uma condição de parada antecipada  que testa se o ponto P mais próximo da origem coincide com a origem~\cite{linahan2015geometricgjk}.

Este subalgoritmo constroi iterativamente um polígono dentro da CSO que tenta incluir a origem. Se o polígono que construímos contém a origem, então podemos dizer que a CSO contém a origem, logo há interseção entre os dois objetos. Este subalgoritmo começa com um simplex inicial e o refina iterativamente adicionando pontos encontrados usando a função de suporte em uma direção que aponta para a origem. Esse processo continua até que a origem seja encontrada dentro do simplex, indicando uma colisão, ou até que se determine que a origem não está contida, o que significa que não há colisão.

Para cada dimensão do simplex Q, o subalgoritmo de Ericson calcula se a origem está contida ou não no simplex usando regiões de Voronoi, remove pontos antigos do simplex e procura por uma nova direção que maximize as chances do próximo ponto de suporte conter a origem. Para cada dimensão é feito uma conta diferente, neste trabalho iremos trabalhar apenas com objetos em 2D, por isso trataremos apenas dos casos 1-simplex e 2-simplex. 

The challenge lies in drawing the optimal simplex to represent the MS. Consider figure 6 below where the MS contains the origin, and while its blue simplex triangle also contains the origin its red one does not.
Press enter or click to view image in full size

% Fortunately, the optimal simplex can be determined algorithmically. First we choose a direction at random represented as a vector, then take the vertex of the MS furthest from the origin in that direction. This vertex is called a support point (Lindemann 2009:2). Next, we take the direction vector from the first support point to the origin, and find the support point which maps to that direction. Finally, we connect these two points in a line, get another direction vector which is normal to this line, then get the support point which maps to this direction. With three support points obtained we will have created a triangular simplex which has a reasonably good chance of containing the origin (if it lies within the MS boundary in the first place). This process is demonstrated in figure 7.

% After this process there remains a possibility that the origin contained in the MS may not have been contained in the initial simplex. To guarantee finding a simplex which will contain the origin we simply use the direction vector from the normal of the simplex edge closest to the origin to find another support point and draw a new simplex using the support points that form the simplex edge closest to the origin. If the MS contains the origin then the origin will also be contained in this new simplex, confirming that a collision has occurred. This is demonstrated in figure 8 below. On the other hand, if the support point found by the normal direction vector happens to be a support point used for the current simplex, then there can be no new simplexes closer to the origin to encapsulate it, confirming that the origin is not within the MS boundary and that no collision has occurred (Reducible 2021).
% % Para detalhar essa rotina usaremos uma conveção de nomear os pontos do simplex em ordem alfabética, com o ponto mais recente sendo A.

Na primeira rodada do laço do Algorimto~\ref{alg:gjk}, depois de encontrar o ponto de suporte, ele é adicionado ao simples Q formando uma linha, este é o caso de 1-simplex ( \textit{ClosestPointOnEdgeToPoint()}). A próxima direção a ser procurada é a direção normal por AB, o ponto de suporte é adicionado ao simplex formando um triangulo.

Como tem possibilidade da origem não estar contida no simplex formado pelo triangulo, caimos no caso 2 simplex (\textit{ClosestPointOnTriangleToPoint()})ilustrada pela Figura~\ref{fig:gjk-ericson-voronoi}. Calculamos uma nova direção de busca a partir da normal da aresta mais próxima a origem, caso  o novo ponto de suporte encontrado seja um dos pontos do simplex, então a origem não pode estar contida no simplex, caso contrário, o ponto A (mais antigo) é excluido e o novo ponto de suporte é adicionado ao simplex e o processo se repete.

testa para dois tipos de região de Voronoi, região de Vértice ou região de Aresta.
\begin{enumerate}
...
No entanto, o processo de encontrar o simplex ideal elimina automaticamente a exigência de verificar quatro dessas regiões. Como o ponto de suporte A foi o primeiro a ser adicionado, B foi o segundo e C foi o terceiro. O ponto de suporte C foi adicionado como resultado de encontrar o vetor direcional normal para a borda AB. Isso significa que a região de Voronoi $E_{AB}$, que fica em frente ao ponto de suporte C através da borda AB não pode conter a origem. As três regiões de canto também não podem conter a origem, uma vez que essas regiões devem situar-se além dos três pontos de apoio do simplex, que foram derivados dos pontos no limite do CSO que maximizam as distâncias em algumas direções determinadas. Isso deixa apenas as regiões de Voronoi $E_{AC}$, $E_{BC}$ e $ABC$ para investigar.

\begin{figure}[htb]
	\centering
	\includesvg[width=\linewidth]{gjk-ericson-voronoi.svg}
	\caption{Região de Voronoi para o caso 3-simplex.}
