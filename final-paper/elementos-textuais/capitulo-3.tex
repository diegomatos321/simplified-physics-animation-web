...
Neste trabalho, detalharemos a versão aprimorada do GJK proposta por \citeonline{gjk_enhanced} que é capaz de tratar qualquer objeto convexo. Além disso, com o passar dos anos, o subalgoritmo de distância de Johnson foi trocado a favor de outros mais sofisticados como, por exemplo, o subalgoritmo de Ericson apresentado na SIGGRAPH 2004~\cite{ericson2004gjk}, que se baseia numa abordagem geométrica ao invés de algébrica, com otimização usando regiões de Voronoi.
...
Assim, o objetivo do GJK reduz para determinar se a CSO contém ou não a origem. A grande coisa do algoritmo é que você não precisa calcular todo CSO, apenas uma amostra do CSO usando um mapeamento suporte. 

Uma função de suporte é definida tal que: seja A um conjunto qualquer, o suporte de A na direção $\vec{d}$ retorna o ponto que pertence a superfície de A mais distante da origem na direção $\vec{d}$, esse ponto é chamado de ponto de suporte. Matematicamente fazemos $S_A (\vec{d}) = max\{v \cdot a : a \in A\}$.
...
A busca pelo ponto mais próximo da origem dentro do CSO é sustentado pelo teorema de Carathéodory [Rockafellar96]. O teorema afirma que, para um corpo convexo H de Rd, cada ponto de H pode ser expresso como a combinação convexa de no máximo d + 1 pontos de H. Isso permite que o algoritmo GJK busque no CSO, mantendo um conjunto Q de até d + 1 pontos de C em cada iteração e Q forma um simplex dentro de C. Se a origem estiver contida no simplex atual, A e B se intersectam e o algoritmo para.

Um Simplex é definido como um politopo de k+1 vérticos em um espaço de k-dimensão. Dessa forma um ponto é 0-simplex, um segmento de reta é 1-simplex, um triângulo é 2-simplex, um tetraedro é 3-simplex, como na Figura~\ref{fig:simplex}.
