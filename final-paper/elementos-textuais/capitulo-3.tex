...
Para politopos, o GJK termina após um número finito de iterações. Para objetos curvos, um termo $\epsilon$ deve ser adicionado para evitar loops infinitos e limitar o erro entre a distância de separação calculada e real. Foi estabelecido em  
%adicionar citação
...
\begin{algorithm}[htb]
	...
	\While{$||P||^2 - P \cdot V > \epsilon^2$}{
		$Q' \leftarrow Q \cup \{V\}$ \\
		P $\leftarrow$ MinimumNormPoint($Q'$) \\
...
O Algoritmo~\ref{alg:gjk} começa com um ponto arbitrário $P$ no CSO e um conjunto simplex vazio $Q$. Então calcula um ponto de suporte $V$ na direção $-P$. Se $V$ não estiver mais distante na direção $-P$ do que o próprio $P$, então $P$ deve ser o ponto mais próximo da origem: o algoritmo termina com $||P||$ como a distância do CSO à origem. Caso contrário, o algoritmo adiciona $V$ ao conjunto simplex atual $Q$ e atualiza $P$ para ser o ponto mais próximo da origem dentro do novo simplex (MinimumNormPoint($Q'$)). O simplex $Q$ é reduzido ao menor subconjunto convexo de $Q$ que ainda contém $P$, e o ponto de suporte $V$ na direção $-P$ é atualizado para a próxima iteração.

% \textbf{DEFINIR REGIÃO VORONOI}

\subsection*{Subalgoritmo de distância de Ericson}

A versão de \citeonline{ericson2004gjk} substituiu a versão algébrica de Johnson por uma geométrica, usando rotinas geométricas primitivas como \textit{ClosestPointOnEdgeToPoint()} e \textit{ClosestPointOnTriangleToPoint()}, como apresentado no Algoritmo~\ref{alg:ericson_gjk}, que é implementado usando operações vetoriais. Apesar disso seu algoritmo é matematicamente equivalente ao de Johnson com um pequeno acrescimo de otimização: uma condição de parada antecipada  que testa se o ponto P mais próximo da origem coincide com a origem~\cite{linahan2015geometricgjk}.
...
% \begin{algorithm}[htb]
% 	\caption{Subalgoritmo de distancia de Ericson}
% 	\KwIn{Simplex Q, ponto P}
% 	\KwOut{}

% 	\If {length(Q) == 3} {
%         return ClosestPointOnTriangleToPoint(Q, p)
% 	}
% 	\Else {
%         return ClosestPointOnEdgeToPoint(Q, p);
% 	}

% 	\label{alg:ericson_gjk}
% \end{algorithm}
...
Para o caso de 1-simplex temos a rotina \textit{ClosestPointOnEdgeToPoint()} que procura o ponto mais próximo

Para o caso de 2 simplex temos a rotina \textit{ClosestPointOnTriangleToPoint()}, ilustrada pela Figura~\ref{fig:gjk-ericson-voronoi}, testa para dois tipos de região de Voronoi, região de Vértice ou região de Aresta.
\begin{enumerate}
