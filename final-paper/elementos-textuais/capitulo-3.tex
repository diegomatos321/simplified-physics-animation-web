...
A versão de \citeonline{ericson2004gjk} substituiu a versão algébrica de Johnson por uma geométrica, usando rotinas geométricas primitivas como \textit{ClosestPointOnEdgeToPoint()} e \textit{ClosestPointOnTriangleToPoint()}, que é implementado usando operações vetoriais como apresentado no Algoritmo~\ref{alg:ericson_gjk}. Apesar disso seu algoritmo é matematicamente equivalente ao de Johnson com um pequeno acrescimo de otimização: uma condição de parada antecipada  que testa se o ponto P mais próximo da origem coincide com a origem~\cite{linahan2015geometricgjk}.
...
Para o caso de 1-simplex temos a rotina \textit{ClosestPointOnEdgeToPoint()} que testa se a origem está contida numa aresta

Para o caso de 2 simplex temos a rotina \textit{ClosestPointOnTriangleToPoint()} que testa para dois tipos de região de Voronoi, região de Vértice ou região de Aresta
\begin{enumerate}
	\item O origin está contida numa região de Vértice. Equação de exemplo para $V_A$:
		\begin{itemize}
			\item $AX \cdot AB \le 0$
			\item $AX \cdot AC \le 0$
		\end{itemize}
	\item O origin está contida numa região de Aresta. Equação de exemplo para $E_{AB}$:
		\begin{itemize}
